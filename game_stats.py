...
class GameStats:
    ...
    def _load_last_levels(self):
        """从文件中加载最后玩的关卡"""
        try:
            with open('last_level.json', 'r') as f:
                data = json.load(f)
    ...
    def save_last_level(self, level):
        ...
        with open('last_level.json', 'w') as f:
    ...
    def _save_max_level_reached(self):
        ...
        try:
            ...
            with open('max_level_reached.json', 'w') as f:
                json.dump(data, f)
    ...
    def _load_max_level_reached(self):
        """从文件加载最高通关关卡"""
        try:
            with open('max_level_reached.json', 'r') as f:
                data = json.load(f)
    ...
    def _load_total_play_time(self):
        """从文件中加载总游戏时长"""
        try:
            with open('total_play_time.json', 'r') as f:
                data = json.load(f)
                return data if isinstance(data, int) else 0
        except (FileNotFoundError, json.JSONDecodeError):
            return 0
    ...
    def save_total_play_time(self):
        """保存总游戏时长到文件"""
        with open('total_play_time.json', 'w') as f:
